...
export const buildOnSearchMergedResponse = async (
  ...
  return buildOnSearchResponse(
    response.searchRes,
    body,
    response?.itemRes?.[0]?.data?.jobs,
    response?.itemRes?.[1]?.data?.jobs
  );
};

export const buildOnSearchResponse = (
  response: any = {},
  body: any = {},
  savedItems = [],
  appliedItems = []
  ...
  const finaldata = bpps.map((bpp: any) => {
    ...
    const providers = bpp?.message?.catalog?.providers;

    const jobProviderPlatform = bpp?.message?.catalog?.descriptor?.name;
    ...
    const jobResults = providers?.map((provider: any) => {
      return {
        company: {
          ...
          imageLink: provider?.descriptor?.images?.map((image: any) => ({
            url: image?.url,
        ...
        jobs: provider?.items?.map((item: any) => ({
          ...
          additionalDesc: {
            url: item?.descriptor?.additional_desc?.url,
            contentType: item?.descriptor?.additional_desc?.content_type
          },
          userSavedItem: !!savedItems?.find(
            (savedItem: any) => savedItem?.job_id == item?.id
          ),
          userAppliedItem: !!appliedItems?.find(
            (appliedItem: any) => appliedItem?.job_id == item?.id
          ),
          locations: provider?.locations
            ?.filter((location: any) =>
              item?.location_ids?.find((id: any) => id == location?.id)
            )
            ?.map((location: any) => ({
              id: location?.id,
    ...
    return { context, jobProviderPlatform, jobResults };
  });

  return { data: finaldata };
};

export const buildSavedAppliedJobResonse = (
  savedResponse: any = {},
  appliedResponse: any = {}
) => {
...
export const buildOnSelectResponse = (response: any = {}, body: any = {}) => {
  ...
  items?.forEach((item: any) => {
    const job: any = {
      ...
      locations: provider?.locations
        ?.filter((location: any) =>
          item?.location_ids?.find(
            (locationId: any) => location.id == locationId
          )
        )
      ...
      educationalQualifications: item?.tags
        ?.filter((tag: any) =>
          tag?.descriptor?.name?.toLowerCase()?.includes("qualifications")
        )
        ?.map((tag: any) => ({
          category: tag?.descriptor?.name,
          qualification: tag?.list?.map((li: any) => ({
    ...
    const workExperience = item?.tags?.find(
      (tag: any) => tag?.descriptor?.name?.toLowerCase() == "work experience"
    );
    const responsibilities = item?.tags?.find(
      (tag: any) => tag?.descriptor?.name?.toLowerCase() == "responsibilities"
    );
    const employmentInformation = item?.tags?.find(
      (tag: any) => tag?.descriptor?.code == "employment-info"
    );
    const compensation = item?.tags?.find(
      (tag: any) => tag?.descriptor?.code == "salary-info"
    );

    job.workExperience = {
      key: workExperience?.descriptor?.name,
      experience: workExperience?.list?.map((li: any) => ({
        code: li?.descriptor?.code,
        name: li?.descriptor?.name,
        value: li?.value
      }))
    ...
    job.compensation = {
      ...
      salaryInfo: compensation?.list?.map((li: any) => ({
        code: li?.descriptor?.code,
        name: li?.descriptor?.name,
        value: li?.value
      }))
        ...
        return {
          ...
          customer: {
            person: {
              ...
              languages: data?.jobApplicantProfile?.languages?.map(
                (language: any) => {
...
export const buildOnInitResponse = (response: any = {}) => {
  ...
  const additionalFormData = Object.entries(xinput?.data ?? {})?.map(
    ([key, value]: any[]) => ({ formInputKey: key, formInputValue: value })
  );
      ...
      fulfillments: [
        {
          ...
          customer: {
            person: {
              ...
              languages:
                input?.confirmation?.jobApplicantProfile?.languages?.map(
                  (language: any) => ({ code: language })
                ),
              URL: input?.confirmation?.jobApplicantProfile?.url,
              creds: input?.confirmation?.jobApplicantProfile?.creds.map(
                (cred: any) => cred
              ),
              tags: [
                {
                  code: "func_skills",
                  list: input?.confirmation?.jobApplicantProfile?.skills?.map(
                    (skill: any) => ({ name: skill })
                  )
                }
              ]
            }
...
export const buildOnStatusResponse = (response: any = {}) => {
  ...
  const jobFulfillments = input?.message?.order?.fulfillments.map(
    (fulfilment: any) => ({
      ...
      jobStatus: {
        status: fulfilment?.state?.descriptor?.name,
        statusCode: fulfilment?.state?.descriptor?.name
      }
    })
  );
...
export const buildCancelRequest = (input: any = {}) => {
  ...
  const message = {
    ...
    cancellation_reason_id: input?.message?.cancellation_reason_id
  };
  return { payload: { context, message } };
};

export const buildCancelResponse = (input: any = {}) => {
